<template>
    <footer>
        <div class="grid grid-cols-2 py-20 gap-28">
            <div class="space-y-5">
                <div class="flex gap-2">
                    <h3 class="text-2xl font-bold text-white">Hollow <span class=" text-secondary">ee</span>nd</h3>
                    <svg class="w-7 lg:w-9" viewBox="0 0 31 31" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path
                            d="M8.39575 7.75016C9.64867 7.75016 10.8112 8.396 11.767 9.53266C12.4774 8.77058 13.317 8.21516 14.2083 7.94391V5.16683C14.2083 4.48169 14.4804 3.82461 14.9649 3.34014C15.4494 2.85567 16.1064 2.5835 16.7916 2.5835H19.3749V5.16683H16.7916V7.94391C17.6828 8.21516 18.5224 8.77058 19.2328 9.53266C20.1887 8.396 21.3512 7.75016 22.6041 7.75016C25.8333 7.75016 28.4166 12.0902 28.4166 17.4377C28.4166 22.7852 25.8333 27.1252 22.6041 27.1252C21.3512 27.1252 20.1887 26.4793 19.2328 25.3427C18.1866 26.4793 16.8949 27.1252 15.4999 27.1252C14.1049 27.1252 12.8133 26.4793 11.767 25.3427C10.8112 26.4793 9.64867 27.1252 8.39575 27.1252C5.16659 27.1252 2.58325 22.7852 2.58325 17.4377C2.58325 12.0902 5.16659 7.75016 8.39575 7.75016ZM11.6249 12.9168L10.0103 15.8231H13.2395L11.6249 12.9168ZM19.3749 12.9168L17.7603 15.8231H20.9895L19.3749 12.9168ZM10.3333 21.9585H12.9166L14.2083 20.6668L15.4999 21.9585H18.0833L19.3749 20.6668L20.6666 21.9585L22.6041 18.0835L17.9799 18.8843L16.7916 20.6668L15.4999 19.3752H12.9166L11.6249 20.6668L10.3333 19.3752L7.74992 18.0835L10.3333 21.9585Z"
                            fill="white" />
                    </svg>
                </div>
                <p class="leading-loose tracking-widest text-white w-[80%]">Lorem ipsum dolor sit amet consectetur
                    adipisicing elit. Assumenda autem nostrum ipsa quaerat vitae! Obcaecati eveniet ipsam deleniti illum
                    consequatur? Optio, adipisci recusandae eius facilis voluptate asperiores eaque quasi ut.</p>
                <img class="w-12 -translate-x-10" src="../assets/image/footer2-img.webp " alt="">
            </div>
            <div class="grid grid-cols-3 mt-2 text-white">
                <div class="">
                    <h3 class="mb-5 text-xl font-bold">About</h3>
                    <div class="flex flex-col gap-3">
                        <a href="#">About Us</a>
                        <a href="#">Feature</a>
                        <a href="#">Career</a>
                    </div>
                </div>
                <div class="">
                    <h3 class="mb-5 text-xl font-bold">Company</h3>
                    <div class="flex flex-col gap-3">
                        <a href="#">Our Team</a>
                        <a href="#">Partnet</a>
                        <a href="#">Questions</a>
                    </div>
                </div>
                <div class="">
                    <h3 class="mb-5 text-xl font-bold">Service</h3>
                    <div class="flex flex-col gap-3">
                        <a href="#">Support</a>
                        <a href="#">Feedback</a>
                        <a href="#">Contact</a>
                    </div>
                </div>
            </div>
        </div>
        <div class="grid my-20 place-items-center">
            <div class="flex items-center gap-5">
                <a href="#" class="grid w-12 h-12 p-2 rounded-lg bg-secondary place-items-center">
                    <img src="../assets/image/social/fb.svg" alt="" class="h-full">
                </a>
                <a href="#" class="grid w-12 h-12 p-2 rounded-lg bg-secondary place-items-center">
                    <img src="../assets/image/social/instagram.svg" alt="" class="h-full">
                </a>
                <a href="#" class="grid w-12 h-12 p-2 rounded-lg bg-secondary place-items-center">
                    <img src="../assets/image/social/twitter.svg" alt="" class="h-full">
                </a>
            </div>
            <div class="mt-5 text-xl font-medium text-white">&copy; All right reserved.</div>
        </div>
    </footer>
</template>

<script setup>

</script>

<style scoped></style>